...
const ROOM_TYPES = [
  ...
  { id: "Food", label: "Food", icon: "🍲", color: "#FFD166" },
  ...
  return (
    <div
      style={{
        ...
        fontFamily: "ui-sans-serif, system-ui", padding: 8,
        transform: open ? "translateY(0)" : "translateY(70%)",
        transition: "transform 180ms ease",
        borderTop: "1px solid rgba(255,255,255,0.12)", zIndex: 10
      }}
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            style={{ padding: "4px 10px", background: tab === "RoomTypes" ? "#1f2937" : "transparent", color: "white", borderRadius: 6 }}
          >
            Room types
          </button>
          <button
            onClick={() => setTab("Objects")}
            style={{ padding: "4px 10px", background: tab === "Objects" ? "#1f2937" : "transparent", color: "white", borderRadius: 6 }}
          >
            Objects
          </button>
        </div>
        <span style={{ opacity: 0.8, fontSize: 12 }}>Drag an item onto the canvas</span>
      </div>

      <div style={{ whiteSpace: "nowrap", overflowX: "auto", paddingBottom: 6 }}>
          ...
          {items.map((it) => (
            <div
              ...
              style={{
                ...
                padding: "8px 10px", borderRadius: 10, cursor: "grab", userSelect: "none"
              }}
              title={it.label}
            >
              <span style={{ fontSize: 18 }}>{(it as any).icon}</span>
              {"color" in it ? (
                <span style={{ width: 14, height: 14, borderRadius: 4, background: (it as any).color, display: "inline-block" }} />
              ) : null}
